...
router.get('/get/summaryOrder/:year/:month', (req, res) => {
  ...
  sql.query('SELECT DATE_FORMAT(orderDate,"%Y-%m-%d") AS orderDate, SUM(totalAmount) AS sumTotalAmount FROM `ORDERS` WHERE MONTH(orderDate)=' + m + ' AND YEAR(orderDate)='+y+' GROUP By DATE_FORMAT(orderDate,"%Y-%m-%d") ORDER BY orderDate', function (error, results, fields) {
    if (error) throw error;
    return res.send({ error: false, data: results, message: 'order list.' });
  });
...
router.get('/get/orderByDate/:searchDate', (req, res) => {
  var d = moment(req.params.searchDate).format("YYYY-MM-DD");
  console.log(d);
